    ...
    <header className="glass rounded-2xl px-6 py-3 flex justify-between items-center mb-6 shrink-0">
      <div className="flex items-center gap-3">
        <button onClick={onExit}
          className="flex items-center justify-center w-10 h-10 rounded-xl glass hover:bg-white/10 transition-all" title="Назад">
      ...
      <div className="flex items-center gap-4">
        <div className="hidden sm:flex items-center">
          <div className="flex -space-x-2">
            {users.slice(0, 3).map((user) => (
              <div key={user.userId}
                className={`w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold border-2 border-gray-900 ${user.userId === hostId ? 'bg-yellow-500/50' : getAvatarColor(user.userName)}`}
                title={user.userName}>
        ...
        <button onClick={copyRoomCode}
          className="hidden md:flex items-center gap-2 glass px-3 py-2 rounded-xl hover:bg-white/10 transition-all group" title="Нажмите чтобы скопировать">
          {isPrivate && (
          ...
          <code className="font-mono text-sm text-white group-hover:text-purple-300 transition-colors">{roomId}</code>
          {copied ? (
